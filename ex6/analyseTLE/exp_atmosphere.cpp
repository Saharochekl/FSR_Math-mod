#include "exp_atmosphere.h"

exp_atmosphere::exp_atmosphere() : atmosphere()
{
    scaleHeight=NULL;
    refHeight=NULL;
    refDensity=NULL;
...
bool exp_atmosphere::getDensity(double *pos, double *d, double t, int count)
{
   ...
   for (i = 0; i < count; ++i)
   {
      loc[0] = pos[ i*6 ] - centralBodyLocation[0];
      loc[1] = pos[i*6+1] - centralBodyLocation[1];
      loc[2] = pos[i*6+2] - centralBodyLocation[2];
      ...
      density[i] = refDensity[index] * exp(-(height - refHeight[index])/scaleHeight[index]);
   }
   return true;
}

int  exp_atmosphere::FindBand(Real height)
{
...
void exp_atmosphere::SetConstants()
{
   if (scaleHeight)
      delete [] scaleHeight;
