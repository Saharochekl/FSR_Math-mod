//$Id: ExponentialAtmosphere.cpp 11032 2012-11-01 17:46:25Z wendys-dev $
//------------------------------------------------------------------------------
...
ExponentialAtmosphere::~ExponentialAtmosphere() {
  if (scaleHeight) delete[] scaleHeight;
  if (refHeight) delete[] refHeight;
  if (refDensity) delete[] refDensity;
}
...
bool ExponentialAtmosphere::Density(Real *position, Real *density, Real epoch,
  ...
  if (centralBodyLocation == NULL)
    throw AtmosphereException(
        "Exponential atmosphere: Central body vector "
        "was not initialized");
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (height < 0.0)
      throw AtmosphereException(
          "Exponential atmosphere: Position vector "
    ...
    if (smoothDensity)
      ...
      density[i] = refDensity[index] *
                   exp(-(height - refHeight[index]) / scaleHeight[index]);
  }
...
void ExponentialAtmosphere::SetConstants() {
  ...
  if (!scaleHeight)
    throw AtmosphereException(
        "Unable to allocate scaleHeight array for "
  ...
  if (!refHeight) {
    ...
    scaleHeight = NULL;
    throw AtmosphereException(
        "Unable to allocate refHeight array for "
        "ExponentialAtmosphere model");
  }
  ...
  if (!refDensity) {
    ...
    refHeight = NULL;
    throw AtmosphereException(
        "Unable to allocate refDensity array for "
        "ExponentialAtmosphere model");
  }
